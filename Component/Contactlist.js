...
export default class Contactlist extends Component {
    ...
    Contacts.getAll().then((list) => {
      ...
      setTimeout(() => {
        for (var i = 0; i < this.state.contactphonenumber.length; i++) {
          this.state.contactphonenumber[i] = this.state.contactphonenumber[
            i
          ].replace(/\s/g, '');
        }
      }, 100);
  ...
  getcontactlist = () => {
    ...
    setTimeout(() => {
      var contactphonenumber = this.state.contactphonenumber.reduce(
    ...
    setTimeout(async () => {
      ...
      for (var i = 0; i < this.state.userdata.length; i++) {
        for (var j = 0; j < this.state.contactphonenumber.length; j++) {
          if (
            this.state.userdata[i].phonenumber ==
            this.state.contactphonenumber[j]
          ) {
              ...
              <FlatList
                ...
                renderItem={({item}) => (
                  <TouchableHighlight
                    activeOpacity={0.6}
                    delayPressIn={0}
                    onPress={() => this.checkexistance(item)}
                    underlayColor={buttonbackground}>
                    <View
                      style={{
                      ...
                      <View
                        ...
                        {item.smallimage == 'No Photo' ? (
                          <View
                            style={{
                              ...
                              borderRadius: (widthsize * 12) / 100 / 2,
                        ...
                        <View
                          style={{flex: 1, marginLeft: (widthsize * 3) / 100}}>
                          <Text
                            allowFontScaling={false}
                            style={{
                              color: textcolor,
